...
export default function LandingModel () {
  ...
  return (
    <Canvas frameloop="demand" style={{pointerEvents: 'auto', cursor:'pointer'}} camera={{ position: [0, -80, 120], 
      // <Canvas frameloop="demand" style={{pointerEvents: 'auto', cursor:'pointer'}} pixelRatio={[1, 1]} camera={{ position: [-15, 27, 150],
      fov: 90, far: 500, near: 10}}>
      ...
      <OrbitControls enableRotate={false} enablePan={false} autoRotate={true} autoRotateSpeed={0.5} minDistance={120} maxDistance={150} />
    </Canvas>
  );
}