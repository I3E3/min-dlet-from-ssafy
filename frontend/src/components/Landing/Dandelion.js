...
export default function Dandelion ({seed}) {
  ...
  const [copiedScene, globalValue, globalValue2] = useMemo(() => [scene.clone(), parseInt(Math.random(seed) * 50 - 25), parseInt(Math.random(seed) * 50 - 25)], [scene])
  const dandle = useRef()
  ...
  return (
    <instancedMesh position={[globalValue * 3 - globalValue2 * 3 + 25, globalValue * 3 + globalValue2 * 3 + 30, globalValue * 4 - globalValue2 * 2]} ref={dandle}>
